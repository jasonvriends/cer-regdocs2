...
``regdocs_3_docling_worker.py --document-id ...``. If the child exits abnormally
(including a segfault or OOM kill), this launcher records the attempt and
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"Docling complete: {len(done)}/{len(current)} current documents succeeded; "

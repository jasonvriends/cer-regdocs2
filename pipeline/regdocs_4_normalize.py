...
PARSER_VERSION = "regdocs-normalizer-2026-08-07-v1"
...
def stable_json(value: Any) -> str:
    """Canonical one-line JSON used for deterministic JSONL output and hashing."""
    return json.dumps(
        value,
        ensure_ascii=False,
        sort_keys=True,
...
# ---------------------------------------------------------------------------
# Candidate selection / artifact resolution
# ---------------------------------------------------------------------------
...
def select_candidates(
    ...
    sql = f"""
        SELECT
            ...
            a.raw_json_path,
            a.markdown_path,
            a.page_count AS db_page_count,
            a.table_count AS db_table_count,
            a.section_count AS db_section_count,
            a.warning_count AS db_warning_count
        FROM analyses a
...
def resolve_artifact(
    ...
    possibilities.append(
        ...
        / candidate.document_id
        / f"{candidate.file_sha256}.{extension}"
    )
...
# ---------------------------------------------------------------------------
# Content Understanding geometry / structure helpers
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Paragraph / section / table / figure normalization
# ---------------------------------------------------------------------------
...
def table_rows(table: dict[str, Any]) -> tuple[list[str], list[int]]:
    ...
    for cell in table.get("cells") or []:
        ...
        if cell.get("kind") in {"columnHeader", "stubHead"}:
            header_rows.add(r)
    lines = ["\t".join(row).rstrip() for row in matrix]
    return lines, sorted(header_rows)
...
def load_and_validate_artifacts(
    ...
    if top_analyzer != candidate.analyzer_id:
        raise ValueError(
            f"Analyzer mismatch: DB={candidate.analyzer_id!r}, JSON={top_analyzer!r}"
        )
    ...
    if md_path is not None:
        ...
        if markdown_file != markdown_from_json:
            raise ValueError(
                f"Markdown artifact differs from markdown embedded in raw JSON: {md_path}"
            )
...
def build_chunks_tables_provenance(
    ...
    for content_index, content in enumerate(result.get("contents") or []):
        ...
        # Detailed table records are written once regardless of how many search chunks they create.
        ...
        def emit_table(table_idx: int, section_path: list[str]) -> None:
            ...
            groups = split_table_for_search(table, target_words, max_words)
        ...
        def emit_figure(figure_idx: int, section_path: list[str]) -> None:
            ...
            figure_text = "\n".join(texts).strip()
            # Avoid indexing logos/single-label decorative figures while preserving maps/diagrams.
            ...
            for pointer in section.get("elements") or []:
                ...
                if typ == "paragraphs" and idx is not None and idx < len(paragraphs):
                    ...
                    pending.append(
                        TextUnit(
                            text=text,
                            evidence=[paragraph_evidence(idx, p)],
                            regions=regions,
                            span=p.get("span"),
                        )
                    )
        ...
        # Walk roots in span order. Any orphan sections are then visited in span order.
        ...
        # CU normally places all useful content under sections. Preserve any orphan tables/figures.
...
def normalize_document(
    ...
    # Per-document deterministic fingerprint across all normalized projections.
    h = hashlib.sha256()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--db",
        default=portable_path(DATABASE_PATH),
        help="REGDOCS SQLite ledger",
    )
    parser.add_argument(
        "--analysis-dir",
        default=portable_path(ANALYZE_DIR),
        help="Stage 3 analysis root",
    )
    parser.add_argument(
        "--output-dir",
        default=portable_path(NORMALIZE_DIR),
        help="Normalized JSONL output directory",
    )
    ...
    parser.add_argument(
        "--document-id",
        action="append",
        help="Normalize only this document ID; may be supplied more than once",
    )
    ...
    parser.add_argument("--skip-errors", action="store_true",
                        help="Continue and emit a partial corpus if a document fails")
    parser.add_argument("--dry-run", action="store_true",
                        help="Select and resolve candidates without writing JSONL or ledger state")
...
def run(args: argparse.Namespace) -> int:
    ...
    print(
        f"Selected {len(candidates)} successful current analysis record(s) "
        f"for {args.analyzer_id} / {args.api_version}."
    )
    if not candidates:
        con.close()
        return 0
    ...
    try:
        ...
        for i, candidate in enumerate(candidates, start=1):
            ...
            try:
                ...
                print(
                    f"          OK pages={p_count} chunks={c_count} tables={t_count} "
                    f"hash={normalized.output_sha256[:12]}"
                )
            ...
            update_run_progress(
                ...
                total_chunks,
                total_tables,
                f"Normalized {candidate.document_id}; {succeeded} succeeded, {failed} failed",
            )

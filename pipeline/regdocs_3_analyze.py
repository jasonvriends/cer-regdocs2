...
Cost controls, credentials, recovery, large-PDF ranging, and artifact contracts
are documented in ``pipeline/regdocs_3_analyze.md``.
"""
...
def pdf_page_count(path: Path) -> int:
    """Return the number of PDF pages without rendering page content."""
    try:
...
def page_ranges(
    ...
    """Build inclusive 1-based page ranges for Azure Content-Range analysis."""
    if page_count < 1:
        return []
    return [
...
def range_artifact_paths(
    ...
    """Return raw-result and metadata paths for one completed page range."""
...
def _reuse_range_artifacts_enabled() -> bool:
    """Respect force/reconciliation CLI semantics for independently saved range parts."""
    return not any(
...
def load_reusable_range_artifact(
    ...
    """Load a prior completed range only when all processing identity fields match."""
...
def analyze_one(
    ...
    """Analyze one file; PDFs over 300 pages are ranged and recombined automatically."""
    ...
    if page_count <= MAX_PAGES_PER_ANALYSIS:
        # Hash was already checked above, so avoid hashing the same PDF twice.
        return _analyze_one_single_request(
            client,
            candidate,
            file_path,
    ...
    for chunk_index, (start_page, end_page) in enumerate(ranges, start=1):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                # Persist every successful range immediately so a later range failure can resume.
    ...
    # Keep each range's Azure contents separate. Stage 4 already iterates contents[]
    # independently, so this preserves original spans without rewriting offsets.

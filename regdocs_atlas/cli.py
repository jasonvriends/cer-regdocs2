...
from .runtime.presentation import banner, format_progress, run_line, stage_label
...
def _cost_monitor(provider: str, before_run_id: int, stop: threading.Event, mode: str) -> None:
    ...
    while not stop.wait(2.0):
        try:
            ...
            # Cost visibility must never make analysis fail.
...
def _finalize_provider_cost(provider: str | None, before_run_id: int, mode: str) -> None:
    ...
    try:
        ...
        if provider == "azure":
            ...
            snapshot = persist_local_compute_cost(DATABASE_PATH, run_id)
...
def _run_legacy(script_name: str, args: Sequence[str], *, stage: str, provider: str | None = None) -> int:
    ...
    header = banner(stage, provider=provider, log_path=str(PIPELINE_LOG_PATH.relative_to(PROJECT_ROOT)))
    print(header)
    ...
    lock_context = (
        ...
        if read_only
        else ProcessLock(PIPELINE_LOCK_PATH, role=f"pipeline:{mode.lower()}")
    )
    ...
    if DATABASE_PATH.is_file():
        ...
        try:
            ...
            if table_exists(con, "runs"):
                ...
                rows = con.execute(
                    f"""
                    SELECT id,stage,status,started_at,finished_at,{release_expr},progress_message,
...
def _cost_command(args: Sequence[str]) -> int:
    ...
    if run_id is None:
        run_id = latest_provider_run(DATABASE_PATH, "azure")
    if run_id is None:

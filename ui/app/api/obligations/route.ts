import { getObligations, hasIntelligenceScope, type IntelligenceScope } from "@/lib/intelligence";
...
export async function GET(request: Request) {
  ...
  if (!hasIntelligenceScope(requestedScope)) {
    return Response.json(
      { error: "Obligation requests require a document, filing, company, or project scope." },
      { status: 400 },
    );
  }
  const requestedTop = Number.parseInt(url.searchParams.get("top") || "200", 10);
  try {
    const obligations = await getObligations(requestedScope, Number.isFinite(requestedTop) ? requestedTop : 200);
    return Response.json({ count: obligations.length, obligations });
  } catch (error) {
    return publicErrorResponse("api.obligations", error, "Obligations retrieval failed.", 502, requestedScope);
  }
}
